...
14. **CSS 버전 관리:** `header.php`에 `const CSS_VERSION` 상수 정의. 모든 CSS/JS 링크에 `?v=<?= CSS_VERSION ?>` 파라미터 추가. 버전 관리 규칙:
    - **패치 버전 (마지막 자리)**: UI 작은 수정, 버그 픽스, 스타일 미세 조정 등 자주 올림 (3.0.9 → 3.0.10 → 3.0.11 ... 제한 없음)
    - **마이너 버전 (중간 자리)**: 전체 페이지 리디자인, 새로운 섹션 추가, 주요 기능 변경 등 큰 변화 (3.0.x → 3.1.0)
    - **메이저 버전 (첫 자리)**: 전체 디자인 시스템 개편, 구조적 변화 등 완전 전면 개편 (3.x.x → 4.0.0)
...
> **일정 페이지 날짜 선택 개선** (2026-03-04): 상단 날짜 바를 "전체보기", "오늘", "1일차 (6/1)", "2일차 (6/2)" 형태로 리디자인. API `GET /api/schedule/days`에서 여행 start_date~end_date 범위에 맞게 `schedule_days`를 자동으로 생성 (date 없는 여행은 기존방식 유지). 일정 추가 모달 드롭다운에서 "새 일차 추가" 옵션 제거, "1일차 (6/1 일)" 형태로 표시. "전체보기" 칩 선택 시 모든 일차의 아이템을 일차별 섹션으로 묶어 표시. `buildDateBar()`/`selectAll()`/`renderAllTimeline()` 함수 신규. 스와이프는 개별 일차 선택 시에만 동작. CSS_VERSION 4.0.6.
> **홈 페이지 전면 재구성** (2026-03-04): 여행 상태별(`tripPhase`: before/during/after/no_date) 히어로 배너(상태별 그라디언트 색상). 여행 전: 준비물·할일 완료율 2열 그리드, 마감 임박 할일(D-Day 배지 urgent/soon/ok), 1일차 일정 미리보기. 여행 중: 오늘 일정 타임라인(PHP 렌더링, JS로 현재 항목 강조), 오늘 지출 카드(통화별), 최근 메모 2개. 여행 후: 총 지출 카드(통화별), 멤버별 지출 바 차트, 완료 현황 카드. 공통: 멤버 칩(PIN 미설정자 표시), "OOO님으로 접속 중" 푸터. 모든 카드·링크가 `<a>` 태그로 감싸 해당 페이지로 이동. `app/pages/home.php`: DATEDIFF(), 상태별 DB 쿼리(urgentTodos, day1Items, todayItems, todayExpenses, recentNotes, totalExpByCur, memberExpenses). `public/assets/css/pages/home.css`: `.home-hero--*`, `.home-progress-grid/card`, `.home-todo-list`, `.home-timeline`(세로선, dot center align), `@keyframes pulse-dot`, `.home-expense-card`, `.home-bar-*`, `.home-member-chip*`. `public/assets/js/pages/home.js` 신규: `getNowMinutes()`, `timeToMinutes()`, `getItemStatus()` (all_day/current/past/future), `updateCurrentSchedule()` (1분마다 갱신, DOM 클래스 토글만). CSS_VERSION 4.1.0.
